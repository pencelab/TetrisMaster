...
public final class Z extends Piece<Z> {

    private Z(final ISquareUnit[] squareUnits, final int initX, final int initY, final Color[] colors) {
    ...
    private Z(final ISquareUnit[] squareUnits, final byte position){
        super(squareUnits, position);
    }

    @Override
    protected Z self(final ISquareUnit[] squareUnits, final byte position) {
        return new Z(squareUnits, position);
    }
    ...
    public static class Builder extends Piece.Builder<Z, Builder> {
        ...
        protected void setDefaults() {
            ...
            for(int i = 0; i < this.colors.length; i++)
                this.colors[i] = Globals.PieceColor.LIME.getColor();
        ...
        public Z build(){
            ...
            Z piece = new Z(new SquareUnit[SQUARE_UNITS], this.x, this.y, this.colors);
            for(int i = 0; i < this.position; i++){
                piece = piece.rotate();
            }
            return new Z(this.recalculateSquareUnitsCoordinates(piece), (byte) this.position);
        }
    }
}
