...
public final class ZR extends Piece<ZR> {

    private ZR(final ISquareUnit[] squareUnits, final int initX, final int initY, final Color[] colors) {
    ...
    private ZR(final ISquareUnit[] squareUnits, final byte position){
        super(squareUnits, position);
    }

    @Override
    protected ZR self(final ISquareUnit[] squareUnits, final byte position) {
        return new ZR(squareUnits, position);
    }
    ...
    public static class Builder extends Piece.Builder<ZR, Builder> {
        ...
        protected void setDefaults() {
            ...
            for(int i = 0; i < this.colors.length; i++)
                this.colors[i] = Globals.PieceColor.ORANGE.getColor();
        ...
        public ZR build(){
            ...
            ZR piece = new ZR(new SquareUnit[SQUARE_UNITS], this.x, this.y, this.colors);
            for(int i = 0; i < this.position; i++){
                piece = piece.rotate();
            }
            return new ZR(this.recalculateSquareUnitsCoordinates(piece), (byte) this.position);
        }
    }
}
