...
public final class Block extends Piece<Block> {

    private Block(final ISquareUnit[] squareUnits, final int initX, final int initY, final Color[] colors) {
    ...
    private Block(final ISquareUnit[] squareUnits, final byte position){
        super(squareUnits, position);
    }

    @Override
    protected Block self(final ISquareUnit[] squareUnits, final byte position) {
        return new Block(squareUnits, position);
    }

    @Override
    protected ISquareUnit[] getRotatedSquareUnitsToPositionZero() {
        ISquareUnit[] squareUnits = {
                new SquareUnit(this.squareUnits[0].getX(), this.squareUnits[0].getY(), this.squareUnits[0].getColor()),
    ...
    public static class Builder extends Piece.Builder<Block, Builder> {

        protected static final int SQUARE_UNITS = 2;
        ...
        protected void setDefaults() {
            ...
            for(int i = 0; i < this.colors.length; i++)
                this.colors[i] = Utils.getRandomPieceColor();
        ...
        public Block build() {
            ...
            Block piece = new Block(new SquareUnit[SQUARE_UNITS], this.x, this.y, this.colors);
            for(int i = 0; i < this.position; i++){
                piece = piece.rotate();
            }
            return new Block(this.recalculateSquareUnitsCoordinates(piece), (byte) this.position);
        }
    }
