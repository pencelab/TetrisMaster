...
public final class L extends Piece<L> {

    private L(final ISquareUnit[] squareUnits, final int initX, final int initY, final Color[] colors) {
    ...
    private L(final ISquareUnit[] squareUnits, final byte position){
        super(squareUnits, position);
    }

    @Override
    protected L self(final ISquareUnit[] squareUnits, final byte position) {
        return new L(squareUnits, position);
    }
    ...
    protected ISquareUnit[] getRotatedSquareUnitsToPositionTwo() {
        ISquareUnit[] squareUnits = {
                ...
                new SquareUnit(this.squareUnits[3].getX(), this.squareUnits[3].getY() - 2, this.squareUnits[3].getColor())
        };
        return squareUnits;
    }
    ...
    protected ISquareUnit[] getRotatedSquareUnitsToPositionThree() {
        ISquareUnit[] squareUnits = {
                ...
                new SquareUnit(this.squareUnits[3].getX() + 2, this.squareUnits[3].getY() + 1, this.squareUnits[3].getColor())
        };
        return squareUnits;
    }
    ...
    public static class Builder extends Piece.Builder<L, Builder> {
        ...
        protected void setDefaults() {
            ...
            for(int i = 0; i < this.colors.length; i++)
                this.colors[i] = Globals.PieceColor.BLUE.getColor();
        ...
        public L build(){
            ...
            L piece = new L(new SquareUnit[SQUARE_UNITS], this.x, this.y, this.colors);
            for(int i = 0; i < this.position; i++){
                piece = piece.rotate();
            }
            return new L(this.recalculateSquareUnitsCoordinates(piece), (byte) this.position);
        }
    }
}
